...
|---|---|---|
| **IAM** | ✅ All existing (users, roles, institutions, ORCID, admin dashboards) + role restructure (3 tiers) + step-4 removal from ORCID onboarding | MFA, SAML, SCIM, pending-approval flow |
...
│   ├── routes/
│   │   ├── auth.py                      # ✅ unchanged
│   │   ├── orcid.py                     # ← MODIFY: remove pending step (see Section 3.1)
...
## 3.1 IAM Changes Required Before Building Modules

These two changes must be made to the **existing** codebase before any prototype module work begins. They are small but structural.

---

### Change 1 — Role Restructure (3 tiers)

DACORIS roles are now organised into three tiers. Update `backend/models.py` — find the existing `ResearchRole` enum and replace it with the expanded version below:

```python
# backend/models.py — REPLACE the existing ResearchRole enum (do not append)

class ResearchRole(str, enum.Enum):
    # ── Research Roles (primary researchers and project staff) ───────────────
    ...
    # ── Administrative Roles (specialist / oversight functions) ─────────────
    DATA_STEWARD            = "data_steward"       # moved from Research tier
    ETHICS_REVIEWER         = "ethics_reviewer"    # moved from Research tier
    ETHICS_CHAIR            = "ethics_chair"
    EXTERNAL_REVIEWER       = "external_reviewer"  # moved from Research tier
```

> **Why three tiers?** Admin-tier roles (Global Admin, Institution Admin) use email+password. Administrative roles (Data Steward, Ethics Reviewer/Chair, External Reviewer) are ORCID-authenticated users with specialist, cross-cutting oversight functions — distinct from primary researchers. Research roles are all other ORCID users. `EXTERNAL_FUNDER` has been removed entirely from the system.

After updating the enum, run a migration:

```bash
cd backend
alembic revision --autogenerate -m "restructure_roles_three_tiers"
alembic upgrade head
```

Update the role list in `backend/routes/institution_admin.py` — find the `list_roles` function and replace its hardcoded list:
...
async def list_roles(
    ...
    """List all available roles grouped by tier"""
    return {
        "research_roles": [
            ...
            {"value": "researcher",               "label": "Researcher / Staff"},
            {"value": "institutional_lead",       "label": "Institutional Lead"},
            {"value": "system_admin",             "label": "System Admin"},
            {"value": "guest",                    "label": "Guest Collaborator"},
            {"value": "applicant",                "label": "Applicant"},
        ],
        "administrative_roles": [
            {"value": "data_steward",             "label": "Data Steward"},
            {"value": "ethics_reviewer",          "label": "Ethics Reviewer"},
            {"value": "ethics_chair",             "label": "Ethics Committee Chair"},
            {"value": "external_reviewer",        "label": "External Reviewer"},
        ],
    }
```

Also update the role assignment dialog in `frontend/app/institution-admin/page.js` to display roles grouped by tier. Find the roles array in the **Role Assignment** dialog and replace it:

```javascript
// In institution-admin/page.js — replace the flat roles array
// with grouped structure that matches the new API response
...
// In the role assignment Select component, render both groups:
// <ListSubheader>Research Roles</ListSubheader>
// {RESEARCH_ROLES.map(...)}
// <ListSubheader>Administrative Roles</ListSubheader>
// {ADMINISTRATIVE_ROLES.map(...)}
```

---

### Change 2 — Remove Step 4 from ORCID Researcher Onboarding

**Current behaviour (Step 4 — to be removed):**
```
Step 1 → Visit Login Page
Step 2 → ORCID Authentication
Step 3 → Institution Selection
Step 4 → Account Status check:
           Auto-approved if domain matches → dashboard
           Pending if domain doesn't match → "awaiting approval" message  ← REMOVE THIS PATH
Step 5 → Access Dashboard
```

**New behaviour (Step 4 removed):**
```
Step 1 → Visit Login Page
Step 2 → ORCID Authentication
Step 3 → Institution Selection → submit
Step 4 → Access Dashboard immediately (always active, no pending state)
```

All researchers are considered trusted upon completing ORCID authentication and selecting their institution. The Institution Admin still manages role assignment, but access is not gated by a pending approval step.

**Edit `backend/routes/orcid.py`** — find the section that creates a new user and sets `PENDING` status, and replace it:

```python
# backend/routes/orcid.py
# FIND this block (around line where is_new_user is handled):
...
if not user:
    ...
    user = User(
        ...
        status=UserStatus.PENDING   # ← REMOVE: was pending, now always active
    )
...
if not user:
    ...
    user = User(
        ...
        orcid_token_expires_at=datetime.utcnow() + timedelta(seconds=expires_in),
        account_type=AccountType.ORCID,
        status=UserStatus.ACTIVE    # ← Always active after ORCID auth
    )
```

Then find the redirect logic at the bottom of the callback and simplify it:
...
if is_new_user and not user.primary_institution_id:
    # New user with no institution yet → send to institution selection
    redirect_url = f"{FRONTEND_URL}/onboarding?token={jwt_token}"
else:
    # All other cases → straight to dashboard
    redirect_url = f"{FRONTEND_URL}/dashboard?token={jwt_token}"
```

**Edit `frontend/app/onboarding/page.js`** — remove the "awaiting approval" / pending state UI branch. The onboarding page should only show the institution selection form and redirect to dashboard on success:

```javascript
// In onboarding/page.js — REMOVE any block that looks like:
// if (status === 'pending') {
//   return <PendingMessage />
// }
// or any rendering of a "pending approval" message.

// The onboarding flow should be:
// 1. Show institution dropdown
// 2. User selects institution → POST /api/onboarding/select-institution
// 3. On success → router.push('/dashboard')
// No pending path needed.
```

**Edit `frontend/app/dashboard/page.js`** — remove any check for `user.status === 'pending'` that shows a "pending approval" message. All authenticated users reaching the dashboard are active.

```javascript
// REMOVE any block like:
// if (user?.status === 'pending') {
//   return <Alert>Your account is awaiting approval...</Alert>
// }
```
...
async def seed():
    async with AsyncSessionLocal() as db:
        ...
        user_data = [
            ...
            # Administrative roles (specialist / oversight)
        ...
        for u in user_data:
            user = User(
                ...
                status=UserStatus.ACTIVE,          # all active — no pending step
        ...
        print("\n  ── Research Roles ──────────────────────────────────────")
        print(f"  {'pi':25s} → pi@demo.ac.ke")
        print(f"  {'grant_officer':25s} → grant@demo.ac.ke")
        print(f"  {'finance_officer':25s} → finance@demo.ac.ke")
        print("\n  ── Administrative Roles ────────────────────────────────")
        print(f"  {'external_reviewer':25s} → reviewer@demo.ac.ke")
        print(f"  {'ethics_chair':25s} → ethics@demo.ac.ke")
        print(f"  {'data_steward':25s} → data@demo.ac.ke")
        ...
        print(f"\nℹ️  All users are ACTIVE — no pending approval step (step 4 removed)")
...
| Role assignment dialog shows flat list | Update to grouped list with Research Roles / Administrative Roles sections (see Section 3.1) |
...
- [ ] **IAM Change 1:** `ResearchRole` enum updated to 3-tier structure (Section 3.1)
- [ ] **IAM Change 1:** `list_roles` in `institution_admin.py` returns grouped tiers (Section 3.1)
- [ ] **IAM Change 2:** `orcid.py` — new users created with `ACTIVE` status, no `PENDING` path (Section 3.1)
- [ ] **IAM Change 2:** redirect logic simplified — onboarding only if no institution, else dashboard (Section 3.1)
- [ ] **IAM Change 2:** `onboarding/page.js` — pending approval UI branch removed (Section 3.1)
- [ ] **IAM Change 2:** `dashboard/page.js` — pending status check removed (Section 3.1)
- [ ] Alembic configured and migrations applied (including role enum migration)
...
*Prototype target: functional demonstration within 2 weeks. Focus is end-to-end scenario coverage, not completeness. Every shortcut listed in Section 11 has a defined resolution path in the full implementation plan (v1.4). IAM changes in Section 3.1 must be completed first — they are prerequisites for all module work.*
